...
describe 'most revenue' do
  before(:each) do
    ...
    @transaction_2 = Transaction.create!(invoice_id: @invoice_1.id, credit_card_number: 9654405418234567, credit_card_expiration_date: '', result: 'success')
    @transaction_3 = Transaction.create!(invoice_id: @invoice_1.id, credit_card_number: 9654405418234567, credit_card_expiration_date: '', result: 'success')
  end
  ...
  it 'returns the top x ranked items by total revenue generated' do
    ...
    expect(most_revenue['data'].count).to eq(2)
    expect(most_revenue['data'][0]['attributes']['name']).to eq(@item_1.name)
