...
describe 'Invoice Transactions' do
  ...
  it 'sends a collection of associated transactions' do
    ...
    expect(invoice_transactions.class).to eq Hash
    expect(invoice_transactions['data'].count).to eq(3)
    expect(invoice_transactions['data'][0]['attributes']['result']).to eq(@transaction_1.result)
    expect(invoice_transactions['data'][1]['attributes']['result']).to eq(@transaction_2.result)
  end
end
