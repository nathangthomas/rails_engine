...
describe 'Invoice Items' do
  ...
  it 'sends a collection of associated invoice items' do
    ...
    expect(invoice_items['data'][0]['attributes'].keys).to eq(['id', 'name', 'description', 'merchant_id', 'unit_price'])
    expect(invoice_items['data'][1]['attributes']['description']).to eq(@item_2.description)
  end
end
