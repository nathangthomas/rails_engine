...
describe 'customer transactions' do
  ...
  it 'returns a collection of associated transactions' do
    ...
    trasactions = JSON.parse(response.body)
    expect(trasactions.class).to eq Hash
    expect(trasactions['data'].count).to eq(2)
    expect(trasactions['data'][0]['attributes']['result']).to eq('success')
  end
end
